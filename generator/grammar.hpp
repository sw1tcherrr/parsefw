...
struct Grammar {
    ...
    using FirstSet  = std::unordered_set<std::string_view>;
	using FirstMap  = std::unordered_map<std::string_view, FirstSet>;
	using FollowSet = std::unordered_set<std::string_view>;
	using FollowMap = std::unordered_map<std::string_view, FollowSet>;
	...
	void MakeFollow() {
		...
		while (changed) {
			changed = false;
			for (auto const& [name, rule] : rules) {
				auto old_size = FOLLOW[name].size();
				for (auto const& production : rule.variants) {
